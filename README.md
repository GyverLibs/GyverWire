...
- `GW_TX_IR <pin, baud = 5000, freq = 38000>` - ИК-канал (модуляция 38 кГц с инверсией)
  - `pin` - пин МК
  - `baud` - скорость 23..500000 бит/с (бод)
  - `freq` - частота модуляции для ИК 8..500000 Гц
...
```cpp
GW_TX_RF<3, 1000> tx(20);   // раскачка 20 мс, скорость 1000
```
...
Для более стабильной и качественной связи рекомендуется снижать скорость до 1000 и ниже

> Максимальная стабильная скорость для пары зелёных модулей (FS1000A и MX-RM-5V): 15'000 бод
...
> Максимальная стабильная скорость: 1'500 бод
